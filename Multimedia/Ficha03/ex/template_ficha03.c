...
void timer(int value)
{
  ...
  if (modelo.bola.y > ALTURA_CAMPO)
  {
    modelo.bola.y = ALTURA_CAMPO;
    modelo.bola.direccao = -modelo.bola.direccao;
  }

  /* Testar colisão raquete 1 */
  if(modelo.bola.x >= modelo.jogador1.x-(LARGURA_RAQUETES/2) 
    && modelo.bola.x <= modelo.jogador1.x+(LARGURA_RAQUETES/2)
    && modelo.bola.y >= modelo.jogador1.y-(ALTURA_RAQUETES/2)
    && modelo.bola.y <= modelo.jogador1.y+(ALTURA_RAQUETES/2)){
    modelo.bola.direccao++;
    printf("bateu p1\n");
  }
...
void timerTeclas(int value)
{
  ...
  if (estado.teclas.a /*&& Condição*/ && modelo.jogador1.y > 0)
  {
    /* Colocar aqui o código necessário */
    modelo.jogador1.y -= 3;
  }
  if (estado.teclas.q /*&& Condição*/ && modelo.jogador1.y < ALTURA_CAMPO)
  {
    /* Colocar aqui o código necessário */
    modelo.jogador1.y += 3;
  }
  if (estado.teclas.l /*&& Condição*/ && modelo.jogador2.y > 0)
  {
    /* Colocar aqui o código necessário */
    modelo.jogador2.y -= 3;
  }
  if (estado.teclas.p /*&& Condição*/ && modelo.jogador2.y < ALTURA_CAMPO)
  {
    /* Colocar aqui o código necessário */
    modelo.jogador2.y += 3;
  }
